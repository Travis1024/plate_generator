...
char_dict = "鄂鄂鄂京沪津渝冀晋蒙辽吉黑苏浙皖闽赣鲁豫鄂湘粤桂琼川贵云藏陕甘青宁新"
char_alp = "ABCDEFGHJKLMNPQRSTUVWXYZ"
char_alp2 = "ABCDEFGHJKLMNPQRSTUVWXYZ0123456789012345678901234567890123456789"
cnt = 10000
yellow = 7
blue = 7
green = 8
...
# 随机透视变换
factor_up_down = 10
factor_left_right = 30
factor_true = 5

# 模糊程度高
# gaussianradius = [2,2,2,2,2,2,2,2,1,3]
# 模糊程度低
gaussianradius = [1,0,1,0,1,0,0,1,2,0]
...
def parse_args():
    ...
    parser.add_argument('--plate-number', default='湘A9999', help='车牌号码')
...
def tfactor(img):
    ...
    hsv[:, :, 0] = hsv[:, :, 0] * (0.85 + np.random.random() * 0.15)
    hsv[:, :, 1] = hsv[:, :, 1] * (0.7 + np.random.random() * 0.7)
...
if __name__ == '__main__':
    ...
    for i in range(cnt):
        ...
        G = R + random.randint(0, 35)
        B = G + random.randint(0, 20)

        if B > 253:
            B = random.randint(245, 254)
        ...
        for p in range(80):
            for q in range(240):
                if (img[p][q][0] > 250 and img[p][q][1] > 250 and img[p][q][2] > 250):
                    flagindex[p][q][0] = 0
                    # img[p][q][0] = B + random.randint(-8,8)
                    # img[p][q][1] = G + random.randint(-15,15)
                    # img[p][q][2] = R + random.randint(-10,10)
                    # if img[p][q][1] > 253:
                    #     img[p][q][1] = random.randint(240, 254)
                    # if img[p][q][0] > 253:
                    #     img[p][q][0] = random.randint(240, 254)
        # 增加高斯模糊
        ...
        cv2.imencode('.png', img)[1].tofile('F:/SmartSite/plate_generator_master/blueplate2/' + platenumber + '.png')
